...
static void stepper_acceleration_update(motion_ctx_t *motion_ctx, motion_action_t *self, MotionEvent event, bool initial_run){
  ...
  float t_next = get_step_time(axis_ctx, axis_ctx->acceleration, fabs(axis_ctx->velocity), axis_ctx->step_distance / axis_ctx->steps_per_mm, axis_ctx->direction);
...
void stepper_acceleration_done(motion_ctx_t *motion_ctx, motion_action_t *self, MotionEvent event){
  ...
  const float step_delta = (t * axis_ctx->initial_velocity + 0.5 * axis_ctx->acceleration * t * t) * axis_ctx->steps_per_mm;
...
static void stepper_cruise_update(motion_ctx_t *motion_ctx, motion_action_t *self, MotionEvent event, bool initial_run){
  ...
  if(initial_run){
    ...
    axis_ctx->velocity = motion_ctx->block->programmed_rate;
    axis_ctx->step_distance = calc_remaining_step(axis_ctx, axis_ctx->direction);
  } else {
    set_motor_step(axis_ctx->idx);
    update_axis_position(axis_ctx, axis_ctx->step_distance);
  ...
  float t_next = fabs(STEPPER_I2S_SAMPLE_RATE * ((axis_ctx->step_distance / axis_ctx->steps_per_mm) / axis_ctx->velocity));
...
void stepper_cruise_done(motion_ctx_t *motion_ctx, motion_action_t *self, MotionEvent event){
  ...
  const float step_delta = t * axis_ctx->velocity * axis_ctx->steps_per_mm;
...
static void stepper_deceleration_update(motion_ctx_t *motion_ctx, motion_action_t *self, MotionEvent event, bool initial_run){
  ...
  float t_next = get_step_time(axis_ctx, -1 * axis_ctx->acceleration, fabs(axis_ctx->velocity), axis_ctx->step_distance / axis_ctx->steps_per_mm, axis_ctx->direction);
...
static float position_update_callback(void *ctx){
  ...
  return axis_ctx->position / axis_ctx->steps_per_mm;
}


void stepper_motion_setup(motion_ctx_t *motion_ctx){
  for(int i=0; i < N_STEPPERS; i++){
    const Axis axis_idx = steppers[i].axis;
    ...
    steppers[i].position = axis_state.position[axis_idx] * steppers[i].steps_per_mm;
    steppers[i].initial_position = steppers[i].position;
    steppers[i].final_position = (axis_state.position[axis_idx] + motion_ctx->block->axis_distance[axis_idx]) * steppers[i].steps_per_mm;
...
void stepper_init(){
  for(int i=0; i < N_STEPPERS; i++){
    ...
    steppers[i].steps_per_mm = 400.0;
  }

  steppers[0].axis = X_AXIS;
  steppers[1].axis = Y_AXIS;
  steppers[2].axis = Z_AXIS;
}
